...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Elke FeatureTypeStyle is een soort vlak die uitgebreid moet worden met de nieuwe kleuren -->
<!--    <xsl:template match="sld:UserStyle">
        <xsl:copy>
            <xsl:apply-templates select="se:FeatureTypeStyle">
                <xsl:sort select="./se:Name"/>
            </xsl:apply-templates>
        </xsl:copy>
    </xsl:template>-->

    <!-- Elke Rule van een FeatureTypeStyle heeft een eigen kleur -->
<!--    <xsl:template match="se:FeatureTypeStyle">
        <xsl:copy>
            <xsl:apply-templates select="se:Rule"/>
        </xsl:copy>
    </xsl:template>-->
...
<!--        <xsl:choose>
    ...
    <!-- Vervang de verwijzing naar de png -->
    <xsl:template match="se:OnlineResource" mode="rule">
        <xsl:param name="kleur"/>
        <xsl:element name="se:OnlineResource">
            <xsl:attribute name="xlink:href">
                <xsl:value-of
                    select="concat(substring(@xlink:href, 0, string-length(@xlink:href) - 5), $kleur/symboolcode/text(), '.png')"
                />
            </xsl:attribute>
    ...
    <!-- Vervang de kleurcode van de lijn -->
    <xsl:template match="se:SvgParameter[@name = 'stroke'][text() != '#000001']" mode="rule">
        <xsl:param name="kleur"/>
        <xsl:element name="se:SvgParameter">
            <xsl:attribute name="name">stroke</xsl:attribute>
